...
namespace FoodStamps
{
    public class MyEmpyrionMod : ModInterface
    {
        ModGameAPI GameAPI;
        public string ModVersion = "FoodStamps v0.0.1";
        public static string ModPath = "..\\Content\\Mods\\FoodStamps\\";
        ...
        public void Game_Start(ModGameAPI gameAPI)
        {
            ...
            try
            {
                string[] LastUseFile = File.ReadAllLines(ModPath + "LastUseLog.txt");
                foreach (string UsageData in LastUseFile)
                {
                    string[] LastUsage = UsageData.Split(' ');
                    LastUseLog.Add(LastUsage[0], LastUsage[1]);
                }
                CommonFunctions.Debug("Success reading LastUseLog.txt");
            }
            catch
            {
                CommonFunctions.Debug("Failed reading LastUseLog.txt");
            }
        }
            ...
            try
            {
                switch (cmdId)
                {
                    case CmdId.Event_ChatMessage:
                        ...
                        if (msg == "/foodstamps reinit")
                        {
                            SetupYamlData = SetupYaml.ReadYaml(ModPath + "Setup.yaml");
                        }
                        else if (msg == SetupYamlData.FoodStampsCommand.ToLower())
                        {
                            try
                            {
                    ...
                    case CmdId.Event_Player_Info:
                        ...
                        if (SeqNrStorage.Keys.Contains(seqNr))
                        {
                            ...
                            if (RetrievedData.Requested == "PlayerInfo" && RetrievedData.function == "FoodStamps" && Convert.ToString(Received_PlayerInfo.entityId) == RetrievedData.Match)
                            {
                                ...
                                string Checker = Timestamp2.ToString("yyyy/MM/dd/HH/mm/ss/ffff");
                                string[] CheckerArray = Checker.Split('/');
                                string Checker2 = CheckerArray[0] + "/" + CheckerArray[1] + "/" + CheckerArray[2];
                                if (LastUseLog.ContainsKey(Convert.ToString(Received_PlayerInfo.entityId)))
                                {
                                    if (LastUseLog[Convert.ToString(Received_PlayerInfo.entityId)] == Checker2)
                                    {
                                        API.Chat("Player", Received_PlayerInfo.entityId, "FoodStamps: You Have already claimed your foodstamps today");
                                    }
                                    else
                                    {
                                        foreach (SetupYaml.Usage DayOfMonth in SetupYamlData.DayOfMonth)
                                        {
                                            if (CheckerArray[2] == Convert.ToString(DayOfMonth.Day))
                                            {
                                                ItemStack[] Stack = CommonFunctions.ReadItemStacks(ModPath + DayOfMonth.FileName);
                                                API.OpenItemExchange(Received_PlayerInfo.entityId, DayOfMonth.WindowHeader, DayOfMonth.WindowSubHeader, "Close", Stack);
                                                LastUseLog[Convert.ToString(Received_PlayerInfo.entityId)] = Checker2;
                                                CommonFunctions.Log(Checker + " " + Received_PlayerInfo.entityId + " " + Received_PlayerInfo.playerName);
                                            }
                                        }
                                    }
                                }
        ...
        public void Game_Exit()
        {
            ...
            File.WriteAllText(ModPath + LastUseLog, "");
            foreach (string player in LastUseLog.Keys)
            {
                File.AppendAllText(ModPath + "LastUseLog.txt", player + " " + LastUseLog[player]);
            }
        }
    }
}
